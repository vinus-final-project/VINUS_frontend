...
const usePrinter = () => {
    ...
    /* 영수증 출력 — 성공 여부 반환. 실패에도 throw 하지 않음.
     * 각 단계별 try/catch 로 한 단계 실패가 뒤 단계로 전파되지 않음. */
    const printReceipt = async (text) => {
        ...
        try {
            ...
            if (!UsbPrinter || typeof UsbPrinter.connect !== "function") {
                console.warn("[printer] UsbPrinter 플러그인 미로드 — 스킵");
                return false;
            }
            ...
            try {
                ...
                console.error("[printer] connect 실패 — 스킵:", e);
                setError(e?.message || "프린터 연결에 실패했습니다.");
                return false;
            }
            ...
            console.error("[printer] 예기치 못한 실패:", err);
